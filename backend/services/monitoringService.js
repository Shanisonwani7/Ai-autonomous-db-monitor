...
const { analyzeSeries } = require("../utils/trendCalculator");
...
// Service methods resolve to { statusCode, body };
// controllers just pass it through.
...
function formatUptime(interval) {
  ...
  if (interval.years) {
    parts.push(`${interval.years} Years`);
  }

  if (interval.months) {
    parts.push(`${interval.months} Months`);
  }
...
async function refreshConnectionStatus(database) {
  ...
  try {
    ...
    const commits = Number(
      statsRow.xact_commit || 0
    );

    const rollbacks = Number(
      statsRow.xact_rollback || 0
    );

    const blocksRead = Number(
      statsRow.blks_read || 0
    );
    ...
    const activeConnections = Number(
      connectionResult.rows[0].total
    );

    const uptime = formatUptime(
      uptimeResult.rows[0].uptime
    );
...
async function getDatabaseVersion(id, userId) {
  ...
  try {
    const database =
      await getOwnedDatabase(id, userId);
...
// that an unexpected error returns a proper JSON 500 instead of an unhandled rejection.
async function getDashboardMetrics(id, userId) {
  try {
    ...
    const result =
      await refreshConnectionStatus(database);
  ...
  try {
    ...
    const result = await client.query(
      ...
      [
        ...
        RUNNING_QUERIES_LIMIT,
      ]
    );

    const queries = result.rows.map(
      (row) => {
        const {
          duration,
          query_preview,
...
async function getDatabaseLocks(id, userId) {
  ...
  try {
    ...
    return errorResultWithMessage(
      err,
      "Failed to fetch database locks"
    );
  } finally {
    if (client) {
      await client.end().catch(() => {});
...
async function getSlowQueries(id, userId) {
  ...
  try {
    ...
    const slowQueries = result.rows.map(
      (row) => {
        const {
          duration,
          query_preview,
...
async function getLongRunningTransactions(
  id,
  userId
) {
  let client;
  ...
  try {
    ...
    return errorResultWithMessage(
      err,
      "Failed to fetch long running transactions"
    );
  } finally {
    if (client) {
      await client.end().catch(() => {});
    ...
    return okResult({
      ...
      summary: {
        database: {
          ...
          activeConnections:
            connectionsResult.rows[0].total,
        },

        monitoring: {
          runningQueries:
            runningQueries.rows[0].total,
          idleSessions:
            idleSessions.rows[0].total,
          slowQueries:
            slowQueries.rows[0].total,
          longTransactions:
            longTransactions.rows[0].total,
          locks:
            locks.rows[0].total,
        },
        ...
        statistics: {
          commits: Number(
            statistics.rows[0].xact_commit
          ),
          rollbacks: Number(
            statistics.rows[0].xact_rollback
          ),
          deadlocks: Number(
            statistics.rows[0].deadlocks
          ),
        },
      },
    });
...
async function collectHistoricalMetrics(
  ...
  try {
    ...
    const commits =
      Number(data.commits || 0);

    const rollbacks =
      Number(data.rollbacks || 0);

    const deadlocks =
      Number(data.deadlocks || 0);

    const blocksRead =
      Number(data.blocks_read || 0);

    const blocksHit =
      Number(data.blocks_hit || 0);

    const runningQueries =
      Number(data.running_queries || 0);

    const slowQueries =
      Number(data.slow_queries || 0);

    const idleSessions =
      Number(data.idle_sessions || 0);

    const longTransactions =
      Number(data.long_transactions || 0);
...
// Trend calculation needs chronological order:
// oldest → newest
      ...
      body: {
        ...
        trends: {
          ...
          metrics: {
            ...
            activeConnections: analyzeSeries(
              metrics.map(
                (metric) =>
